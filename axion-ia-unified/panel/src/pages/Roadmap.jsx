...
export default function Roadmap({ embedded = false }) {
  ...
  const [novaIdeia, setNovaIdeia]   = useState({ titulo: "", descricao: "", prioridade: 3, complexidade: "media", categoria: "" });
  ...
  async function adicionarIdeia() {
    if (!selecionado || !novaIdeia.titulo.trim()) return;
    setMsg("");
    try {
      const r = await apiFetch(`/roadmap/${selecionado._id}/item`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setMsg(`✅ Ideia "${novaIdeia.titulo}" adicionada ao roadmap.`);
      setNovaIdeia({ titulo: "", descricao: "", prioridade: 3, complexidade: "media", categoria: "" });
      setModalIdeia(false);
      await carregarDetalhe(selecionado._id);
    } catch { setMsg("Erro ao adicionar ideia."); }
  }
  ...
  return (
    <div className={embedded ? "" : "page-container"}>
      ...
      <div className="toolbar" style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 16 }}>
        ...
        {selecionado && (
          <button onClick={() => setModalIdeia(true)}
      ...
      {modalIdeia && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: 440, maxWidth: "90vw", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
            <h3 style={{ margin: "0 0 16px", fontSize: 16 }}>💡 Adicionar Ideia ao Roadmap</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <input placeholder="Título da ideia *" value={novaIdeia.titulo} onChange={e => setNovaIdeia({ ...novaIdeia, titulo: e.target.value })}
                style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #ccc", fontSize: 14 }} />
              <textarea placeholder="Descrição (opcional)" value={novaIdeia.descricao} onChange={e => setNovaIdeia({ ...novaIdeia, descricao: e.target.value })}
                rows={3} style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #ccc", fontSize: 13, resize: "vertical" }} />
              <div style={{ display: "flex", gap: 10 }}>
                <select value={novaIdeia.prioridade} onChange={e => setNovaIdeia({ ...novaIdeia, prioridade: Number(e.target.value) })}
                ...
                <select value={novaIdeia.complexidade} onChange={e => setNovaIdeia({ ...novaIdeia, complexidade: e.target.value })}
                  style={{ flex: 1, padding: "6px 10px", borderRadius: 6, border: "1px solid #ccc", fontSize: 13 }}>
              ...
              <input placeholder="Categoria (ex: UX, Performance, BI)" value={novaIdeia.categoria} onChange={e => setNovaIdeia({ ...novaIdeia, categoria: e.target.value })}
                style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #ccc", fontSize: 13 }} />
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 16 }}>
              <button onClick={() => setModalIdeia(false)}
                style={{ padding: "6px 16px", background: "#e9ecef", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 13 }}>Cancelar</button>
              <button onClick={adicionarIdeia} disabled={!novaIdeia.titulo.trim()}
                style={{ padding: "6px 16px", background: "#f59e0b", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600, fontSize: 13, opacity: novaIdeia.titulo.trim() ? 1 : 0.5 }}>Salvar Ideia</button>
            </div>
          </div>
        </div>
      )}
