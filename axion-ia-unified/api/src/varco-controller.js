...
export async function listarFrota(req, res) {
  try {
    // Tentar buscar do VARCO Cloud se credenciais disponíveis
    ...
    if (!existsSync(dataFile)) {
      return res.status(404).json({ erro: "Dados de frota não disponíveis. Execute a análise primeiro ou configure VARCO_EMAIL/VARCO_PASSWORD.", caminhoEsperado: dataFile });
    }
...
export function auditoriaStatus(req, res) {
  try {
    // Corrigido: caminho relativo correto (../ para subir um nível)
    const dataFile = resolve(process.cwd(), "../auditoria-itscam/analise-dados.json");

    if (!existsSync(dataFile)) {
      return res.status(404).json({ erro: "Dados de auditoria não encontrados. Execute a análise primeiro.", caminhoEsperado: dataFile });
    }
