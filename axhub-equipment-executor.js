...
const BASE_URL = 'https://homologacao.axhub.axion.ws';
...
export async function executeEquipmentCycle(data, onProgress = () => {}) {
  ...
  try {
    ...
    await selectKendoComboBox('ModeloEquipamento', `${data.modelo_marca} ${data.modelo_nome}`);
    await selectKendoComboBox('TipoEquipamento', data.tipo_nome);
    await selectKendoComboBox('GrupoEquipamento', data.grupo_nome);
    ...
    // ─────────────────────────────────────────────
    // PASSO 7: Cadastrar Faixa (localização física)
    // ─────────────────────────────────────────────
    ...
    await page.goto(`${BASE_URL}/equipamento`, { waitUntil: 'networkidle0' });
    ...
    await report(
      results.steps[results.steps.length - 1]?.label || 'Erro',
      'error',
      `Falha: ${error.message}`,
      ss
    );
...
/**
 * Schema completo de campos para o ciclo de equipamentos
 * Utilizado pelo CUTI para renderizar o formulario dinamico
 */
export const EQUIPMENT_CYCLE_SCHEMA = {
  ...
  fields: [
    // FABRICANTE
    { name: 'fabricante_nome',      label: 'Fabricante - Nome',            type: 'text',   required: true,  placeholder: 'Ex: AXION TECNOLOGIA',        hint: 'Nome do fabricante do equipamento' },
    { name: 'fabricante_slug',      label: 'Fabricante - Slug (ID)',        type: 'text',   required: false, placeholder: 'Ex: axion-tec',               hint: 'Identificador unico sem espacos (gerado automaticamente se vazio)' },
    ...
    { name: 'modelo_marca',         label: 'Modelo - Marca',               type: 'text',   required: true,  placeholder: 'Ex: VELSIS',                  hint: 'Marca comercial do equipamento' },
    { name: 'modelo_nome',          label: 'Modelo - Nome/Codigo',         type: 'text',   required: true,  placeholder: 'Ex: VSIS-OCR',                hint: 'Modelo especifico do equipamento' },
    ...
    { name: 'modelo_portaria',      label: 'Modelo - Portaria INMETRO (Completa)', type: 'text', required: true, placeholder: 'Ex: PORTARIA INMETRO/DIMEL No 245/2022', hint: 'Referencia completa que aparece na tarja das infracoes' },
    // GRUPO DE EQUIPAMENTOS
    { name: 'grupo_nome',           label: 'Grupo de Equipamentos',        type: 'text',   required: true,  placeholder: 'Ex: GRUPO TESTE 2026',        hint: 'Nome do grupo para organizacao no mapa' },
    { name: 'grupo_cor',            label: 'Grupo - Cor (hex)',            type: 'text',   required: false, placeholder: 'Ex: #0000ff',                 hint: 'Cor de exibicao no mapa (padrao #0000ff)' },
    // EQUIPAMENTO
    { name: 'equip_serie',          label: 'Equipamento - Nr Serie',       type: 'text',   required: true,  placeholder: 'Ex: SN-20260001',             hint: 'Numero de serie fornecido pelo fabricante' },
    { name: 'equip_codigo',         label: 'Equipamento - Codigo',         type: 'text',   required: true,  placeholder: 'Ex: AXT001',                  hint: 'Codigo unico no sistema (aparece na tarja)' },
    { name: 'equip_cert_inmetro',   label: 'Equipamento - Certificado INMETRO', type: 'text', required: true, placeholder: 'Ex: CERT-2026-001',         hint: 'Numero do certificado de afericao INMETRO' },
    ...
    // FAIXA (localizacao fisica)
    { name: 'faixa_codigo',           label: 'Faixa - Codigo',             type: 'text',   required: false, placeholder: 'Ex: FX-AXT001-01',             hint: 'Codigo unico da faixa' },
    { name: 'faixa_numero',           label: 'Faixa - Numero da Faixa',    type: 'number', required: false, placeholder: '1',                            hint: 'Numero inteiro da faixa (1, 2, 3...)' },
    { name: 'faixa_sentido',          label: 'Faixa - Sentido',            type: 'text',   required: false, placeholder: 'Ex: CRESCENTE',                hint: 'Sentido do trafego' },
    { name: 'faixa_logradouro',       label: 'Faixa - Logradouro',         type: 'text',   required: false, placeholder: 'Ex: AV. PAULISTA',             hint: 'Nome da via onde o equipamento esta instalado' },
    ...
    { name: 'faixa_latitude',         label: 'Faixa - Latitude',           type: 'number', required: false, placeholder: 'Ex: -16.6869',                 hint: 'Latitude decimal (negativo para sul)' },
    { name: 'faixa_longitude',        label: 'Faixa - Longitude',          type: 'number', required: false, placeholder: 'Ex: -49.2648',                 hint: 'Longitude decimal (negativo para oeste)' },
  ]
};