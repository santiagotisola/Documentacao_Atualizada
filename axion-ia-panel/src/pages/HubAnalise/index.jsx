...
const MODULOS = [
  ...
  { id: 'depara-equip', icon: '🔄', label: 'Depara Equipamentos', desc: 'Compara equipamentos AxHub × AxCross e identifica diverg�ncias', cor: '#7c3aed' },
...
// "?"?"? Dropdown de seleção de sites (multi-select, pesquisável) "?"?"?"?"?"?"?"?"?"?"?"?"?"?"?"?
function SiteSelectorDropdown({ sitesAtivos, onToggleSite, onLimpar, fechar }) {
  const [filtro, setFiltro] = useState('');
  const [sistemaFiltro, setSistemaFiltro] = useState('todos');

  const sites = useMemo(() => {
    return ALL_CREDENCIAIS.filter(s => {
      const matchSistema = sistemaFiltro === 'todos' || s.sistema?.toLowerCase() === sistemaFiltro;
      const matchFiltro  = !filtro.trim() ||
      ...
      return matchSistema && matchFiltro;
    });
  }, [filtro, sistemaFiltro]);
  ...
  return (
    <div style={{
      ...
      {/* Filtros */}
      <div style={{ padding: '0.75rem', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: '0.5rem' }}>
        <input
          autoFocus
          value={filtro}
          onChange={e => setFiltro(e.target.value)}
          placeholder="🔍 Pesquisar por nome, URL, estado ou tipo..."
          style={{ flex: 1, padding: '0.4rem 0.75rem', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '0.82rem', outline: 'none', background: '#f8fafc' }}
        />
        <select
          value={sistemaFiltro}
          onChange={e => setSistemaFiltro(e.target.value)}
          style={{ padding: '0.4rem 0.5rem', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '0.78rem', background: '#f8fafc', color: '#374151' }}
        >
      ...
      <div style={{ padding: '0.5rem 0.75rem', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
        <span style={{ fontSize: '0.7rem', color: '#9ca3af' }}>{sitesAtivos.length} selecionado(s) · {sites.length} exibido(s)</span>
        {sitesAtivos.length > 0 && (
          <button onClick={onLimpar} style={{ marginLeft: 'auto', fontSize: '0.72rem', padding: '0.2rem 0.5rem', borderRadius: '6px', border: '1px solid #fecaca', background: '#fef2f2', cursor: 'pointer', color: '#dc2626', fontWeight: 700 }}>
            ✕ Limpar seleção
          </button>
        )}
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
          <thead>
            <tr style={{ background: '#f8fafc', position: 'sticky', top: 0, zIndex: 1 }}>
              <th style={{ padding: '0.45rem 0.75rem', textAlign: 'left', fontWeight: 700, color: '#374151', borderBottom: '1px solid #e2e8f0', width: '28px' }}></th>
              <th style={{ padding: '0.45rem 0.75rem', textAlign: 'left', fontWeight: 700, color: '#374151', borderBottom: '1px solid #e2e8f0' }}>Nome</th>
              <th style={{ padding: '0.45rem 0.75rem', textAlign: 'left', fontWeight: 700, color: '#374151', borderBottom: '1px solid #e2e8f0' }}>URL</th>
              <th style={{ padding: '0.45rem 0.75rem', textAlign: 'left', fontWeight: 700, color: '#374151', borderBottom: '1px solid #e2e8f0' }}>UF</th>
              <th style={{ padding: '0.45rem 0.75rem', textAlign: 'left', fontWeight: 700, color: '#374151', borderBottom: '1px solid #e2e8f0' }}>Sistema</th>
            </tr>
          </thead>
          <tbody>
            {sites.map(s => {
              const ativo = sitesAtivos.some(sa => sa.id === s.id);
              return (
                <tr
                  key={s.id}
                  ...
                  style={{
                    cursor: 'pointer',
                    background: ativo ? '#f0f4ff' : 'transparent',
                ...
                >
                  <td style={{ padding: '0.45rem 0.75rem' }}>
                    <div style={{
                      ...
                      border: `2px solid ${ativo ? '#667eea' : '#d1d5db'}`,
                      background: ativo ? '#667eea' : 'white',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                      {ativo && <span style={{ color: 'white', fontSize: '0.55rem', fontWeight: 800 }}>✓</span>}
                    </div>
                  </td>
                  <td style={{ padding: '0.45rem 0.75rem', fontWeight: 600, color: '#1a202c' }}>{s.nome}</td>
                  <td style={{ padding: '0.45rem 0.75rem', color: '#6366f1', fontFamily: 'monospace', fontSize: '0.72rem' }}>{s.url}</td>
                  <td style={{ padding: '0.45rem 0.75rem', color: '#6b7280' }}>{s.estado || '—'}</td>
                  <td style={{ padding: '0.45rem 0.75rem' }}>
                    <span style={{
                      padding: '0.1rem 0.4rem', borderRadius: '4px', fontSize: '0.65rem', fontWeight: 700,
                      background: s.sistema === 'AxHub' ? '#dbeafe' : s.sistema === 'AxCross' ? '#dcfce7' : '#f3e8ff',
                      color:      s.sistema === 'AxHub' ? '#1d4ed8' : s.sistema === 'AxCross' ? '#15803d' : '#7c3aed',
                    }}>{s.sistema}</span>
                  </td>
                </tr>
              );
            ...
            {!sites.length && (
              <tr><td colSpan={5} style={{ padding: '2rem', textAlign: 'center', color: '#9ca3af' }}>Nenhum site encontrado</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div style={{ padding: '0.75rem', borderTop: '1px solid #f1f5f9' }}>
        ...
        >
          {sitesAtivos.length > 0
            ? `Confirmar — ${sitesAtivos.length} site${sitesAtivos.length > 1 ? 's' : ''} selecionado${sitesAtivos.length > 1 ? 's' : ''}`
            : 'Fechar'}
        </button>
      </div>
    </div>
  ...
  return (
    <div className="hub-analise">
      ...
      <div style={{
        ...
        {sitesAtivos.length > 0 && (
          <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap', maxWidth: '320px' }}>
            {sitesAtivos.map(s => (
              <span
                key={s.id}
                onClick={() => toggleSite(s)}
                title={`${s.url} — clique para remover`}
                style={{
                ...
                {filtrados.map(m => {
                  ...
                  return (
                    <div key={m.id} onClick={() => toggle(m.id)} style={{
                      ...
                      <div style={{ width: '18px', height: '18px', borderRadius: '4px', flexShrink: 0, border: `2px solid ${ativo ? m.cor : '#d1d5db'}`, background: ativo ? m.cor : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {ativo && <span style={{ color: 'white', fontSize: '0.6rem', fontWeight: 800 }}>✓</span>}
                      </div>
                    </div>
                  );
                })}
                {filtrados.length === 0 && <div style={{ padding: '1.5rem', textAlign: 'center', color: '#9ca3af', fontSize: '0.82rem' }}>Nenhum módulo encontrado</div>}
              </div>
              <div style={{ padding: '0.75rem', borderTop: '1px solid #f1f5f9' }}>
                <button onClick={() => setSelectorAberto(false)} style={{ width: '100%', padding: '0.5rem', borderRadius: '8px', border: 'none', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer' }}>
        ...
        <div style={{
          ...
          {sitesAtivos.length > 1 && (
            <span style={{ marginLeft: 'auto', fontSize: '0.72rem', color: '#92400e', fontStyle: 'italic' }}>
              Todos os módulos ativos analisarão os {sitesAtivos.length} sites selecionados
            </span>
          )}
        </div>
      )}
