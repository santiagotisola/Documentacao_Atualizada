...
// Mapa de prioridade: Jitbit retorna número (0-3)
const PRIORITY_MAP = {
  0: { label: 'Baixa',   color: '#64748b', bg: 'rgba(100,116,139,0.1)' },
  1: { label: 'Normal',  color: '#6366f1', bg: 'rgba(99,102,241,0.1)'  },
  2: { label: 'Alta',    color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' },
  3: { label: 'Crítica', color: '#ef4444', bg: 'rgba(239,68,68,0.12)'  },
};

function PriorityBadge({ value }) {
  const p = PRIORITY_MAP[value];
  if (!p) return <span style={{ color: '#94a3b8', fontSize: '0.75rem' }}>-</span>;
  return (
    <span style={{
      padding: '2px 8px', borderRadius: 5, fontSize: '0.75rem', fontWeight: 700,
...
export default function Helpdesk() {
  // navigate removido — view "porsite" é inline agora
  ...
  // Insights de demandas por site (de /helpdesk/sites-overview)
  const [insightsData, setInsightsData] = useState(null);
  const [insightsLoading, setInsightsLoading] = useState(false);

  async function carregarInsights() {
    if (insightsData) return; // já carregado
    setInsightsLoading(true);
    try {
      const r = await api.get("/helpdesk/sites-overview");
      setInsightsData(r.data);
  ...
  return (
    <div>
      ...
      <div className="filters-row" style={{ marginBottom: "1.25rem", gap: "0.5rem", flexWrap: "wrap" }}>
        {view === "criar" ? (
          ...
          <>
            ...
            <button className="btn" onClick={() => { setView("insights"); carregarInsights(); }}
              style={{ background: view === "insights" ? "rgba(16,185,129,0.15)" : "var(--surface)", color: view === "insights" ? "#10b981" : "var(--text-muted)", border: `1px solid ${view === "insights" ? "#10b981" : "var(--border)"}`, fontWeight: view === "insights" ? 700 : 400 }}>
              📊 Demandas
            </button>

            <button className="btn" onClick={() => setView("porsite")}
              style={{ background: view === "porsite" ? "rgba(14,165,233,0.15)" : "var(--surface)", color: "#0ea5e9", border: `1px solid ${view === "porsite" ? "#0ea5e9" : "#0ea5e9"}`, fontWeight: view === "porsite" ? 700 : 400 }}>
              🏢 Por Site
            </button>

            {view !== "polling" && view !== "fila" && view !== "sla" && view !== "insights" && (
              <>
            ...
            {(view === "polling" || view === "fila" || view === "detalhe" || view === "sla" || view === "insights" || view === "porsite") && (
      ...
      {/* ===== DEMANDAS POR SITE (Chamados × Sites) ===== */}
      {view === "insights" && (
        <div>
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: "1.1rem", marginBottom: 4 }}>📊 Chamados × Sites — Visão Integrada</h3>
            <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", margin: 0 }}>
              Ranking de demandas por site, categorias mais frequentes e inteligência operacional automática.
            </p>
          </div>

          {insightsLoading && <p style={{ color: "var(--text-muted)" }}>Carregando dados…</p>}

          {!insightsLoading && insightsData && (() => {
            const ranking = insightsData.ranking
              ? [...insightsData.ranking].sort((a, b) => b.abertos - a.abertos)
              : [];
            const kpis = insightsData.kpis || {};
            const naoAssociados = (insightsData.naoAssociados || []).slice(0, 15);
            const miniBar = (value, max, cor) => {
              const pct = max > 0 ? Math.min(100, (value / max) * 100) : 0;
              return (
                <div style={{ height: 6, background: "#334155", borderRadius: 3, overflow: "hidden", minWidth: 60 }}>
                  <div style={{ height: "100%", width: `${pct}%`, background: cor, borderRadius: 3 }} />
                </div>
              );
            };
            const kpiItems = [
              { label: "Total", value: kpis.totalTickets ?? "—", color: "#60a5fa" },
              { label: "Abertos", value: kpis.abertos ?? "—", color: "#f87171" },
              { label: "Críticos", value: kpis.criticos ?? "—", color: "#fca5a5" },
              { label: "Sem Resposta", value: kpis.semResposta ?? "—", color: "#fb923c" },
              { label: "Sites c/ Demanda", value: kpis.sitesComDemanda ?? "—", color: "#a78bfa" },
              { label: "Não Associados", value: kpis.categoriasNaoAssociadas ?? "—", color: "#94a3b8" },
            ];
            const insights = [];
            if (ranking.length > 0 && ranking[0].abertos >= 3) {
              insights.push({ cor: "#f59e0b", bg: "rgba(245,158,11,0.08)", icon: "⚠️", titulo: "Alta Concentração", msg: `${ranking[0].siteId} tem ${ranking[0].abertos} chamados abertos — maior volume do período.` });
            }
            if ((kpis.criticos || 0) > 0) {
              insights.push({ cor: "#ef4444", bg: "rgba(239,68,68,0.08)", icon: "🔴", titulo: "Chamados Críticos", msg: `${kpis.criticos} chamado${kpis.criticos > 1 ? "s" : ""} crítico${kpis.criticos > 1 ? "s" : ""} ativo${kpis.criticos > 1 ? "s" : ""} no momento.` });
            }
            if ((kpis.semResposta || 0) > 0) {
              insights.push({ cor: "#fb923c", bg: "rgba(251,146,60,0.08)", icon: "⏰", titulo: "Sem Resposta", msg: `${kpis.semResposta} chamado${kpis.semResposta > 1 ? "s" : ""} aguardando resposta.` });
            }
            if ((kpis.categoriasNaoAssociadas || 0) > 0) {
              insights.push({ cor: "#94a3b8", bg: "rgba(148,163,184,0.08)", icon: "🔗", titulo: "Tickets Não Mapeados", msg: `${kpis.categoriasNaoAssociadas} categori${kpis.categoriasNaoAssociadas > 1 ? "as" : "a"} sem site associado — revisar no mapa de sites.` });
            }
            if (insights.length === 0 && ranking.length > 0) {
              insights.push({ cor: "#22c55e", bg: "rgba(34,197,94,0.08)", icon: "✅", titulo: "Operação Normal", msg: "Nenhuma anomalia detectada nos parâmetros monitorados." });
            }
            return (
              <>
                {/* KPIs resumo */}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: 10, marginBottom: 20 }}>
                  {kpiItems.map(k => (
                    <div key={k.label} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, padding: "10px 12px", textAlign: "center" }}>
                ...
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px 20px", marginBottom: 20 }}>
                  <div style={{ fontSize: 13, color: "#94a3b8", fontWeight: 700, marginBottom: 12 }}>📊 RANKING DE DEMANDAS POR SITE</div>
                  {ranking.length > 0 ? (
                    <div style={{ overflowX: "auto" }}>
                      <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12 }}>
                        <thead>
                          <tr style={{ background: "var(--bg)" }}>
                            {["Site", "Abertos", "Críticos", "Total", "Carga"].map(h => (
                              <th key={h} style={{ ...slaStyles.th }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {ranking.slice(0, 20).map(r => {
                            ...
                            const cor = pct > 50 ? "#ef4444" : pct > 25 ? "#f59e0b" : "#22c55e";
                            return (
                              <tr key={r.siteId}>
                                <td style={slaStyles.td}><strong>{r.siteId}</strong></td>
                                <td style={slaStyles.td}><span style={{ padding: "2px 8px", background: "#450a0a", color: "#f87171", borderRadius: 10, fontWeight: 700, fontSize: 11 }}>{r.abertos}</span></td>
                                <td style={slaStyles.td}>{(r.criticos || 0) > 0 ? <span style={{ padding: "2px 8px", background: "#7f1d1d", color: "#fca5a5", borderRadius: 10, fontSize: 11 }}>{r.criticos}</span> : <span style={{ color: "#475569" }}>—</span>}</td>
                                <td style={slaStyles.td}>{r.total}</td>
                                <td style={{ ...slaStyles.td, minWidth: 80 }}>{miniBar(pct, 100, cor)}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                    ...
                    <p style={{ color: "#475569", padding: "0.5rem 0" }}>Nenhum dado disponível.</p>
                  )}
                </div>

                {/* Tickets não associados a sites */}
                {naoAssociados.length > 0 && (
                  <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px 20px", marginBottom: 20 }}>
                    <div style={{ fontSize: 13, color: "#94a3b8", fontWeight: 700, marginBottom: 12 }}>🔗 TICKETS NÃO ASSOCIADOS A SITES</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                      {naoAssociados.map((c, i) => (
                        <span key={i} style={{ padding: "4px 12px", background: "#1e293b", border: "1px solid #334155", borderRadius: 12, fontSize: "0.82rem", color: "#e2e8f0" }}>
                          {c.secao ? `${c.secao} — ` : ""}{c.categoriaNome} <strong style={{ color: "#60a5fa" }}>{c.abertos}</strong>
                        </span>
                      ))}
                    </div>
                    <p style={{ fontSize: 11, color: "#475569", margin: "8px 0 0" }}>Esses tickets não possuem site vinculado. Associe via mapa de sites para rastreamento.</p>
                  </div>
                )}

                {/* Insights Automáticos */}
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px 20px" }}>
                  <div style={{ fontSize: 13, color: "#94a3b8", fontWeight: 700, marginBottom: 12 }}>💡 INSIGHTS AUTOMÁTICOS</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    {insights.map((ins, i) => (
                      <div key={i} style={{ borderLeft: `4px solid ${ins.cor}`, background: ins.bg, borderRadius: "0 8px 8px 0", padding: "0.75rem 1rem" }}>
                        <strong style={{ color: ins.cor }}>{ins.icon} {ins.titulo}:</strong>
                        <span style={{ color: "#e2e8f0", marginLeft: 8 }}>{ins.msg}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {view === "sla" && (
        <div>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end", marginBottom: 16 }}>
            ...
            {slaDados && <button onClick={exportarSlaCsv} style={slaStyles.btnSuccess}>📥 Exportar CSV</button>}
          </div>
        ...
        <div style={{ maxWidth: 720 }}>
          ...
          <div className="card" style={{ marginBottom: "1.25rem" }}>
            ...
            <p style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "0.75rem", marginBottom: 0 }}>
              Score {">="} 85% resposta automática • Score {">="} 65% sugestão para humano • Score abaixo de 65% escalado
            </p>
          </div>
      ...
      {/* ===== POR SITE — Visão Agregada de Chamados por Site ===== */}
      {view === "porsite" && (
