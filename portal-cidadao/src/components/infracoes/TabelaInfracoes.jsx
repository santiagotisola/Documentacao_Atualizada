...
const TabelaInfracoes = ({ infracoes }) => {
  ...
  // Navegar para contestação
  const handleContestar = (infracao) => {
    ...
    if (!token) {
      navigate('/login', { 
        state: { 
          from: `/contestacao/${infracao.AutoInfracao}`,
          message: 'Você precisa fazer login para contestar uma infração'
        } 
      });
    } else {
      navigate(`/contestacao/${infracao.AutoInfracao}`, { 
        state: { infracao } 
      });
    }
  };
            ...
            {infracoesOrdenadas.map((infracao) => (
              <tr key={infracao.AutoInfracao} className="hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button
                    onClick={() => handleContestar(infracao)}
...
export default TabelaInfracoes;
