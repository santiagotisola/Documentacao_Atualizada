...
- Name: portal.spec.js >> Consulta de Infrações Anônima >> deve carregar a página home corretamente
- Location: tests\e2e\portal.spec.js:7:3
...
Error: strict mode violation: getByText('Consultar Infrações') resolved to 3 elements:
    1) <a href="/" class="text-sm font-medium transition-colors text-primary-600">Consultar Infrações</a> aka getByRole('navigation').getByRole('link', { name: 'Consultar Infrações' })
    2) <p class="text-gray-600">Informe seu CPF ou placa do veículo para consulta…</p> aka getByText('Informe seu CPF ou placa do')
    3) <a href="/" class="hover:text-white transition-colors">Consultar Infrações</a> aka getByRole('contentinfo').getByRole('link', { name: 'Consultar Infrações' })
